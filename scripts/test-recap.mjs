...
function issue(o = {}) {
  ...
  const built = { key: key || `ACME-${100 + seq}`, id: String(seq), boardId: board, fields };
...
recap = build({
  issues: [
    ...
    issue({ board: 1, status: "Done", key: "ACME-300" }),
    issue({ board: 1, status: "To Do", key: "ACME-200", created: "2026-08-06" }),
    issue({ board: 1, status: "In Code Review", key: "ACME-100", sprint: [{ id: 9, state: "closed" }] }),
    issue({ board: 1, status: "To Do", key: "ACME-999", subtask: true }),
  ],
});
check("sub-tasks are not listed, matching the totals",
  !recap.tickets.some((t) => t.key === "ACME-999"));
check("grouped by board, then by the configured status order, then by key",
  recap.tickets.map((t) => t.key).join() === "ACME-200,ACME-100,ACME-300,PLAT-9");
check("board name resolved for the group heading", recap.tickets[0].board === "ACME");
...
check("scope creep flagged per ticket",
  recap.tickets.find((t) => t.key === "ACME-200").addedAfterStart === true);
check("carry-in flagged per ticket",
  recap.tickets.find((t) => t.key === "ACME-100").carriedIn === true);
check("a done ticket says so, for the status pill",
  recap.tickets.find((t) => t.key === "ACME-300").done === true);
...
const cut = build({
  issues: [
    issue({
      key: "ACME-777", account: "acc-1", name: "Avery Quinn", created: PRE_SPRINT,
...
check("a truncated history names its issue so the page can caveat it",
  cut.activity.truncated.length === 1 && cut.activity.truncated[0] === "ACME-777");
