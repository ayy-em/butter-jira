...
const PROJEACME = (process.env.JIRA_PROJEACME || "")
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean);
...
const jql = PROJEACME.length
  ? `issuetype = Epic AND project in (${PROJEACME.join(",")})`
...
try {
  ...
  const candidates = PROJEACME.length
    ? (boards.values || []).filter((b) => PROJEACME.includes(b.location?.projectKey))
    : boards.values || [];
