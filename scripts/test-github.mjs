...
check("list parsed", list.repos.map(gh.repoSlug).join(",") === "acme/platform-api,acme/web-ui,acme/reporting");
...
check("second alias present", /r1: repository\(owner: "acme", name: "web-ui"\)/.test(query));
...
check("failing repo attributed by alias, not swallowed", activity.failures[0]?.repo === "acme/web-ui");
...
storage = {
  ...
  github: { enabled: true, host: "github.com", org: "acme", repos: ["acme/platform-api", "acme/web-ui"] },
};
...
check("query names both repos", /platform-api[\s\S]*web-ui/.test(requests[0].options.body));
...
storage.github = {
  ...
  repos: ["acme/platform-api", "acme/web-ui"],
};
await cfg.loadConfig();
requests = [];
nextResponse = null;
...
check("failure names the repo", partial.failures[0].repo === "acme/web-ui");
