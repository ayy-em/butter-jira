<p align="center">
  <img src="assets/logo-wordmark.png" alt="ButterJira" width="440">
</p>
...
| `docs/CHANGELOG.md` has a `## vX.Y.Z` section for the tag | A release page that cannot say what changed is a release nobody can decide about. Write the section first; the tag is the last step |
...
[CHANGELOG.md](docs/CHANGELOG.md) verbatim under *What's new*. Not generated from
...
`manifests/base.json` holds everything the three agree on;
`manifests/{chrome,firefox,edge}.json` are the overlays. Edit those, never the
...
assets/favicon*.ico # tab icon for the four pages
...
The same rule sent the rest of the root somewhere: the manifest sources to
`manifests/`, and `PRODUCT.md`, `ROADMAP.md` and `CHANGELOG.md` to `docs/`. What
is left at the top level is there because it has to be. `manifest.json` has no
choice — a browser loading the repo unpacked looks for a manifest at the top
level and nowhere else, and it is generated, so edit `manifests/` and never it.
`README.md` and `LICENSE` are what GitHub renders on the landing page and reads
for the licence badge, and `config.local.example.json` sits beside the file you
copy it to. Everything else has a folder.

## Design system

No CSS framework, no icon package, no component library — see
[PRODUCT.md](docs/PRODUCT.md). What holds the screens together instead is a small set
